  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser && currentUser.email === ADMIN_EMAIL) {
        ...
        // Start listeners only after admin is verified
        ...
        setUser(null);
        // Cleanup listeners if user logs out
  ...
  if (!isFirebaseConfigured) {
    return (
      <div className="min-h-screen bg-primary-dark flex items-center justify-center p-6">
        <div className="max-w-md bg-primary-navy p-10 rounded-3xl border border-white/10 text-center">
          <AlertTriangle size={48} className="text-yellow-500 mx-auto mb-6" />
          <h2 className="text-2xl font-bold text-white mb-4">Admin Unavailable</h2>
          <p className="text-gray-400">Firebase keys are missing. Please configure them to access the admin panel.</p>
        </div>
      </div>
    );
  }
