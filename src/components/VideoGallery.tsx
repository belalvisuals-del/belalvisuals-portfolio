          ...
          {videoItems.map((item) => (
            ...
            >
              <div className="aspect-video relative overflow-hidden">
                <img
                  src={item.thumbnail || `https://img.youtube.com/vi/${item.url.split('v=')[1]?.split('&')[0]}/maxresdefault.jpg`}
                  alt={item.title}
                  className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
                  referrerPolicy="no-referrer"
                  loading="lazy"
                />
                <div className="absolute inset-0 bg-primary-dark/40 group-hover:bg-primary-dark/20 transition-colors duration-300 flex items-center justify-center">
                  <div className="w-16 h-16 bg-primary-light/90 backdrop-blur-md rounded-full flex items-center justify-center text-white transform scale-90 group-hover:scale-100 transition-transform duration-300 shadow-xl shadow-primary-light/30">
                    <Play size={28} fill="currentColor" />
                  </div>
                </div>
              </div>
              <div className="p-6">
                <h3 className="text-lg font-bold text-white group-hover:text-primary-light transition-colors">{item.title}</h3>
                <p className="text-xs text-gray-500 mt-1 uppercase tracking-widest font-semibold">Motion Design</p>
              </div>
            </motion.div>
          ))}
        </div>
